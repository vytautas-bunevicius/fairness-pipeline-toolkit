...
class TestConfigParser:
    ...
    def test_validate_missing_sections(self):
        ...
        for section in expected_missing:
            ...
            assert any("Field required" in error and section in error for error in errors) or any(f"Missing required section: {section}" in error for error in errors)
    ...
    def test_validate_invalid_repair_level(self, valid_config_dict):
        ...
        errors = ConfigParser.validate(config)
        assert any("repair_level must be between 0.0 and 1.0" in error for error in errors)
    ...
    def test_validate_empty_sensitive_features(self, valid_config_dict):
        ...
        errors = ConfigParser.validate(config)
        assert any("At least one sensitive feature must be specified" in error for error in errors)
    ...
    def test_validate_invalid_test_size(self, valid_config_dict):
        ...
        assert any("less than or equal to 0.99" in error for error in errors) or any("test_size must be between" in error for error in errors)
    ...
    def test_validate_unknown_base_estimator(self, valid_config_dict):
        ...
        config = valid_config_dict.copy()
        config['training']['method']['parameters']['base_estimator'] = 'UnknownEstimator'
