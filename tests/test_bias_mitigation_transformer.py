...
from fairness_pipeline_toolkit.pipeline.bias_mitigation_transformer import (
...
class TestBiasMitigationTransformer:
    ...
    def test_fit_no_non_sensitive_features(self, simple_data):
        ...
        with pytest.raises(
            ValueError, match="At least one non-sensitive feature is required"
        ):
    ...
    def test_transform_mean_matching(self, sample_data):
        ...
        transformer = BiasMitigationTransformer(
            sensitive_features=["race"], repair_level=0.8, method="mean_matching"
        )
        transformer.fit(sample_data)
        ...
        pd.testing.assert_series_equal(
            transformed_data["race"], sample_data["race"], check_names=False
        )
        ...
        for feature in transformer.categorical_features_:
            if feature in transformer.non_sensitive_features_:
                pd.testing.assert_series_equal(
                    transformed_data[feature], sample_data[feature], check_names=False
                )

    def test_transform_multivariate_repair(self, sample_data):
        """Test transform with multivariate repair method."""
        transformer = BiasMitigationTransformer(
            sensitive_features=["race"], repair_level=0.6, method="multivariate_repair"
        )
        transformer.fit(sample_data)
    ...
    def test_transform_covariance_matching(self, sample_data):
        ...
        transformer = BiasMitigationTransformer(
            sensitive_features=["race"], repair_level=0.7, method="covariance_matching"
        )
        transformer.fit(sample_data)
    ...
    def test_repair_level_zero(self, sample_data):
        ...
        transformer = BiasMitigationTransformer(
            sensitive_features=["race"], repair_level=0.0
        )
        transformer.fit(sample_data)
    ...
    def test_repair_level_full(self, sample_data):
        ...
        for feature in non_sensitive_features:
            ...
            white_mean = transformed_data[transformed_data["race"] == "White"][
                feature
            ].mean()
            black_mean = transformed_data[transformed_data["race"] == "Black"][
                feature
            ].mean()

            # With full repair, group means should be closer to overall mean
            # than in original data
            original_white_mean = sample_data[sample_data["race"] == "White"][
                feature
            ].mean()
            original_black_mean = sample_data[sample_data["race"] == "Black"][
                feature
            ].mean()

            # Differences should be reduced
            original_diff = abs(original_white_mean - overall_mean) + abs(
                original_black_mean - overall_mean
            )
            transformed_diff = abs(white_mean - overall_mean) + abs(
                black_mean - overall_mean
            )

            assert (
                transformed_diff <= original_diff + 1e-6
    ...
    def test_invalid_method(self):
        ...
        transformer = BiasMitigationTransformer(
            sensitive_features=["race"], method="invalid_method"
        )
