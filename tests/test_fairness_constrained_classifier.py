...
from fairness_pipeline_toolkit.training.fairness_constrained_classifier import (
...
class TestFairnessConstrainedClassifier:
    ...
    def test_init_default_params(self):
        ...
        classifier = FairnessConstrainedClassifier(sensitive_features=["race"])
    ...
    def test_init_invalid_constraint(self):
        ...
        with pytest.raises(ValueError, match="Unknown constraint"):
            FairnessConstrainedClassifier(
                constraint="invalid_constraint", sensitive_features=["race"]
            )

    @patch(
        "fairness_pipeline_toolkit.training.fairness_constrained_classifier.FAIRLEARN_AVAILABLE",
        False,
    )
    ...
    def test_fit_basic(self, sample_data):
        ...
        X, y = sample_data
        classifier = FairnessConstrainedClassifier(sensitive_features=["race", "sex"])
    ...
    def test_fit_with_fairlearn(self, sample_data):
        ...
        with patch(
            "fairness_pipeline_toolkit.training.fairness_constrained_classifier.ExponentiatedGradient"
        ) as mock_eg:
    ...
    def test_get_fairness_info(self, sample_data):
        ...
        X, y = sample_data
        classifier = FairnessConstrainedClassifier(
            constraint="equalized_odds", sensitive_features=["race"]
        )
        classifier.fit(X, y)
    ...
    def test_threshold_optimization_edge_cases(self):
        ...
        classifier = FairnessConstrainedClassifier(sensitive_features=["group"])
        classifier.use_fairlearn = False  # Force fallback method

        # Mock base estimator
        classifier.base_estimator = MagicMock()
        classifier.base_estimator.predict_proba.return_value = np.array(
    ...
    def test_single_class_target(self):
        ...
        X = pd.DataFrame({"feature": [1, 2, 3, 4], "race": ["A", "B", "A", "B"]})
